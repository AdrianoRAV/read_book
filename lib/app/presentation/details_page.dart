...
class BookDetailsPage extends StatelessWidget {
  ...
  BookDetailsPage(this.book) {
    // Chama a função de download automaticamente quando a página é criada
    _downloadBook();
  }
  ...
  Future<void> _downloadBook() async {
    ...
    try {
      ...
      if (appDocDir != null) {
        ...
        if (response.statusCode == 200) {
          await _openEpubViewer(savePath);
        } else {
          print('Erro ao baixar o arquivo EPUB: ${response.statusMessage}');
        }
      } else {
  ...
  Future<void> _openEpubViewer(String filePath) async {
    try {
      VocsyEpub.setConfig(
        themeColor: Colors.blue, // Altere conforme necessário
        identifier: "bookReader",
        scrollDirection: EpubScrollDirection.ALLDIRECTIONS,
        allowSharing: true,
        enableTts: true,
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                  onPressed: () {}, // Desabilita o botão de download
                  child: Text('Download'),
                ),
                ElevatedButton(
                  onPressed: () {
